...
// TTLs (usados también por controlador)
...
export default class CaptacionDateo extends BaseModel {
  ...
  @column() declare origen: Origen
  @column() declare observacion: string | null

  // Imagen (opcional)
  @column({ columnName: 'imagen_url' }) declare imagenUrl: string | null
  @column({ columnName: 'imagen_mime' }) declare imagenMime: string | null
  @column({ columnName: 'imagen_tamano_bytes' }) declare imagenTamanoBytes: number | null
  @column({ columnName: 'imagen_hash' }) declare imagenHash: string | null
  @column({ columnName: 'imagen_origen_id' }) declare imagenOrigenId: string | null
  @column({ columnName: 'imagen_subida_por' }) declare imagenSubidaPor: number | null

  // Consumo
  @column({ columnName: 'consumido_turno_id' }) declare consumidoTurnoId: number | null
  @column.dateTime({ columnName: 'consumido_at' }) declare consumidoAt: DateTime | null
  @column({ columnName: 'payload_hash' }) declare payloadHash: string | null

  // Vínculos comerciales
  @column({ columnName: 'convenio_id' }) declare convenioId: number | null
  @column({ columnName: 'prospecto_id' }) declare prospectoId: number | null
  @column({ columnName: 'vehiculo_id' }) declare vehiculoId: number | null
  @column({ columnName: 'cliente_id' }) declare clienteId: number | null

  @column() declare resultado: ResultadoDateo
  @column({ columnName: 'motivo_no_exitoso' }) declare motivoNoExitoso: string | null
  ...
  // Normalización preventiva (no depende del front)
  @beforeSave()
  ...
  // Computados (útiles si serializas el modelo)
  @computed()
