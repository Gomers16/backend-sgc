...
export default class ContratoEventoController {
  ...
  public async store({ request, response, params }: HttpContext) {
    const contratoId = params.contratoId // --- INICIO DE LÍNEAS DE DEPURACIÓN ---
    console.log('--- INICIANDO CONTROLEVENTS.STORE ---')
    console.log('Contrato ID:', contratoId) // --- FIN DE LÍNEAS DE DEPURACIÓN ---
    ...
    try {
      ...
      console.log('Payload recibido:', payload) // --- FIN DE LÍNEAS DE DEPURACIÓN ---

      if (!contratoId || !payload.tipo || !payload.fechaInicio) {
        // --- INICIO DE LÍNEAS DE DEPURACIÓN ---
        console.log('Error: Payload incompleto') // --- FIN DE LÍNEAS DE DEPURACIÓN ---
        return response.badRequest({
          message: 'Faltan campos requeridos: contratoId, tipo o fechaInicio.',
        })
      }
      const newEvento = new ContratoEvento() // --- INICIO DE LÍNEAS DE DEPURACIÓN ---
